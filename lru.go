...
type TreadsafeLRU[K comparable, V any] struct {
	lru *lru[K, V]
	mu  sync.Mutex
}

func (l *TreadsafeLRU[K, V]) Len() int {
	defer lockUnlock(&l.mu)()

	return l.lru.Len()
}

func (l *TreadsafeLRU[K, V]) Cap() int {
	defer lockUnlock(&l.mu)()

	return l.lru.Cap()
}

func (l *TreadsafeLRU[K, V]) Purge() {
	defer lockUnlock(&l.mu)()

	l.lru.Purge()
}

func (l *TreadsafeLRU[K, V]) Keys() []K {
	defer lockUnlock(&l.mu)()

	return l.lru.Keys()
}

func (l *TreadsafeLRU[K, V]) Values() []V {
	defer lockUnlock(&l.mu)()

	return l.lru.Values()
}

func (l *TreadsafeLRU[K, V]) Contains(key K) bool {
	defer lockUnlock(&l.mu)()

	return l.lru.Contains(key)
}

func (l *TreadsafeLRU[K, V]) Add(key K, val V) (evicted bool) {
	defer lockUnlock(&l.mu)()

	return l.lru.Add(key, val)
}

func (l *TreadsafeLRU[K, V]) GetOldest() (key K, val V, ok bool) {
	defer lockUnlock(&l.mu)()

	return l.lru.GetOldest()
}

func (l *TreadsafeLRU[K, V]) RemoveOldest() (K, bool) {
	defer lockUnlock(&l.mu)()

	return l.lru.RemoveOldest()
}

func (l *TreadsafeLRU[K, V]) Remove(key K) bool {
	defer lockUnlock(&l.mu)()

	return l.lru.Remove(key)
}

func (l *TreadsafeLRU[K, V]) Resize(size int) int {
	defer lockUnlock(&l.mu)()

	return l.lru.Resize(size)
}

func (l *TreadsafeLRU[K, V]) Peek(key K) (V, bool) {
	defer lockUnlock(&l.mu)()

	return l.lru.Peek(key)
}

func (l *TreadsafeLRU[K, V]) Get(key K) (V, bool) {
	defer lockUnlock(&l.mu)()

	return l.lru.Get(key)
}

func NewLRU[K comparable, V any](size int, onEvicted func(K, V)) *TreadsafeLRU[K, V] {
	return &TreadsafeLRU[K, V]{
